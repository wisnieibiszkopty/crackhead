...
if __name__ == "__main__":
    ...
    zipped = zip(messages, keys)

    nonCapLettersCompiler = re.compile('[^A-Z]')

    for m, k in zipped:
        print(m)
        print(k)
        compiledMessage = nonCapLettersCompiler.sub('', m.upper())
        key = vigenere.generateKeyReplication(compiledMessage, k)
        result = vigenere.encrypt(compiledMessage, key)
        print(result)
        print(vigenere.decrypt(result, key))
        print('---------------------')
